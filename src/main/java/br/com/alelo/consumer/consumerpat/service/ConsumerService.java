...
public class ConsumerService {
  ...
  public Consumer updateConsumer(Consumer consumer) throws HibernateException {
    ...
    var currentConsumer = repository.findById(consumer.getId())
  ...
  public void buy(int establishmentType, String establishmentName, int cardNumber, String productDescription,
    ...
    if (establishmentType == EstablishmentType.FOOD_CARD) {
      ...
      consumer.setFoodCardBalance(consumer.getFoodCardBalance() - value);
      if (consumer.getFoodCardBalance() < 0)
        throw new Exception("Insufficient funds");
      else
        repository.save(consumer);

    } else if (establishmentType == EstablishmentType.DRUGSTORE_CARD) {
      consumer = repository.findByDrugstoreNumber(cardNumber);
      if (consumer == null)
        throw new Exception("Consumer not found");
      consumer.setDrugstoreCardBalance(consumer.getDrugstoreCardBalance() - value);
      if (consumer.getDrugstoreCardBalance() < 0)
        throw new Exception("Insufficient funds");
      else
        repository.save(consumer);
      ...
      if (consumer == null)
        throw new Exception("Consumer not found");
      consumer.setFuelCardBalance(consumer.getFuelCardBalance() - value);
      if (consumer.getFuelCardBalance() < 0)
        throw new Exception("Insufficient funds");
      else
        repository.save(consumer);
